...
<p align="center">
  Twitter clone built with ReactJS, Typescript, Tailwind CSS, and a Node/Express backend.
</p>
---
...
**Backend**  
→ Node.js (Express + Prisma + PostgreSQL) — see `backend/`  

**Database**  
→ PostgreSQL (e.g., Dockerized locally, Google Cloud SQL in prod)  
...
The repository now ships with a brand-new TypeScript backend inside [`backend/`](backend/README.md) using Express 5, Prisma, and PostgreSQL. Follow the backend README to configure environment variables, run Prisma migrations, and start the API locally (Dockerized Postgres instructions included). Point `VITE_API_URL` at the backend’s base URL once it is running.
